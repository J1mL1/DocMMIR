...
from transformers import AlignModel, AutoTokenizer, AutoProcessor
...
class ALIGNModel(nn.Module):
    ...
    def __init__(self, mode: str = "image"):
        """
        Initializes the ALIGN model for both image and text encoding.
    ...
    def preprocess_image(self, image_path: str) -> dict:
        """
        Preprocesses a single image from a file path using the ALIGN processor.
    ...
    def preprocess_text(self, texts: List[List[str]]) -> torch.Tensor:
        """
        Preprocesses a list of text inputs using the ALIGN processor.
        ...
        return self.processor(text=texts, padding=True, truncation=True, max_length=256, return_tensors="pt")

    def forward(self, inputs: Union[List[List[str]], List[List[torch.Tensor]]]) -> torch.Tensor:
        """
        Encodes a batch of inputs using ALIGN.
        ...
        try:
            ...
            if self.mode == "image":
                ...
                for image_tensors in inputs:
                    ...
                    avg_embedding = encoded_images.mean(dim=0, keepdim=True) if len(
                        image_tensors) > 1 else encoded_images
    ...
    def get_embedding_size(self) -> int:
        """
        Returns the embedding size of the ALIGN model.
        """
        return 640
